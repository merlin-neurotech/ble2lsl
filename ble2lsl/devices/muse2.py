"""Interfacing parameters for the Muse 2 headband (with PPG).

More information on the data provided by the Muse2 headband can be found
...
STREAMS = ['EEG', 'accelerometer', 'gyroscope', 'telemetry', 'status', 'PPG']
...
PARAMS = dict(
    streams=dict(
        ...
        channel_count=streams_dict([5, 3, 3, 4, 1, 3]),
        nominal_srate=streams_dict([256, 52, 52, 0.1, 0.0, 64]),
        ...
        ch_names=streams_dict([('TP9', 'AF7', 'AF8', 'TP10', 'Right AUX'),
                               ...
                               ('message',),
                               ('PPG1','PPG2','PPG3')]),
        chunk_size=streams_dict([12, 3, 3, 1, 1, 6]),
    ),
    ...
    ble=dict(
        ...
        PPG=['273e000f-4c4d-454d-96be-f03bac821358',
...
PPG_HANDLE_CH_IDXS = {56: 0, 59: 1, 62: 2}
PPG_HANDLE_RECEIVE_ORDER = [56, 59, 62]
"""Channel indices and receipt order of PPG packets."""

class PacketHandler(BasePacketHandler):
    """Process packets from the Muse2 headset into chunks."""
